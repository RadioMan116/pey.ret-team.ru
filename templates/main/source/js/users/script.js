...
$(document).ready(function () {
	...
	// document.querySelector('.product-card__title').append(document.querySelector(".product-card__brand"))
	...
	if (windowWidth2 <= 767) {
		...
		(function () {
			let checkArticle = document.querySelector(".articles.swiper-container");
			if (checkArticle) {
				...
				let articlePagination = checkArticle.querySelector('.swiper-pagination-articles');
				checkArticle.querySelectorAll('.articles__item').length >= 2 ? articlePagination.style.display = 'flex' : articlePagination.style.display = 'none';
			}
		})();
...
$(window).on('load', function () {
	...
	if ($(".comparison").length) {
		...
		if (windowWidth2 > 767) {
			...
			$(".js-swiper-comparison").css({
				'margin-top': ($(".thead__comparison").height() - 3 + 'px')
			});
			$(".comparison .js-swiper-pagination").css({
				'top': ($(".thead__comparison").height() + 29 + 'px')
			});
	...
	if (windowWidth2 > 767) {
		...
		let footerOpen = document.querySelectorAll('.menu-footer');
		footerOpen[0].classList.add('active');
	...
	// document.querySelectorAll('a.special-offers__buy[title="В магазин"]').forEach(link => {
	// 	let url = link.href;
